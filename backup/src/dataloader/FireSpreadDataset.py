    ...
    def read_list_of_images(self):
        ...
        for fire_year in self.included_fire_years:
            ...
            if not self.load_from_hdf5:
                fires_in_year = glob.glob(os.path.join(self.data_dir, str(fire_year), "*"))
                fires_in_year = [f for f in fires_in_year if os.path.isdir(f)]
                fires_in_year.sort()
                for fire_dir_path in fires_in_year:
                    fire_name = os.path.basename(fire_dir_path)
                    fire_img_paths = glob.glob(os.path.join(fire_dir_path, "*.tif"))
                    fire_img_paths.sort()
    ...
    def get_generator_for_hdf5(self):
        ...
        for year, fires_in_year in self.imgs_per_fire.items():
            for fire_name, img_files in fires_in_year.items():
                ...
                img_dates = [os.path.basename(img_path).split("_")[0].replace(".tif", "")
                             for img_path in img_files]
