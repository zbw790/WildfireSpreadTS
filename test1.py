...
class WildFireConfig:
    ...
    BEST_FEATURES = [3, 4, 0, 1, 2, 13, 14, 15, 16, 5, 8, 9, 22]  # NDVI+EVI2+VIIRS+Topo+Landcover+Weather
    
    # Loss configuration - REDUCED for better optimization
    POSITIVE_CLASS_WEIGHT = 50   # Reduced from 236 - too aggressive
    ...
    LEARNING_RATE =30e-4        # Increased for faster convergence
    BATCH_SIZE = 30             # Increased from 4 to utilize 16GB VRAM
    NUM_CROPS = 10              # More crops for better oversampling
    FIRE_CROP_THRESHOLD = 5     # Minimum fire pixels required in selected crop
...
class OfficialFireSpreadDataset(Dataset):
    ...
    def _filter_files_by_years(self):
        ...
        for file_path in self.file_paths:
            # Extract year from filename (assuming format like "fire_event_2020_001.hdf5")
            try:
                filename = os.path.basename(file_path)
                year = int(filename.split('_')[2])  # Adjust based on your naming convention
                if year in self.years:
                    valid_files.append(file_path)
            except:
                # If year extraction fails, include all files (fallback)
    ...
    def _ten_crop_sample(self, input_sequence, target_frame):
        """IMPROVED ten-crop oversampling with guaranteed fire selection"""
        ...
        # Generate MORE random crops for better fire selection
        ...
        if max_fire_count >= self.config.FIRE_CROP_THRESHOLD:
            ...
            if targeted_result is not None:
                ...
                # Last resort: random crop
                best_idx = np.random.randint(len(crops))
                selected_crop = crops[best_idx]
                selected_target = targets[best_idx]
        
        # DEBUG: Print selection stats occasionally
    ...
    def _targeted_fire_sample(self, input_sequence, target_frame):
        """Try to find crops near existing fire pixels"""
        target_fire = target_frame[-1]  # Active_Fire channel
        fire_locations = np.where(target_fire > 0)
        
        if len(fire_locations[0]) == 0:
            return None
        
        # Try to crop around existing fire locations
        target_size = self.config.SPATIAL_SIZE
        T, C, H, W = input_sequence.shape
        ...
        for _ in range(10):  # Try 10 times
            ...
            # Center crop around this fire pixel (with some randomness)
        ...
        # If we found a good crop, return it directly instead of index
...
class ImprovedDiceBCELoss(nn.Module):
    """
    IMPROVED Dice-BCE loss with better class imbalance handling
    """
    
    def __init__(self, pos_weight=50, dice_weight=0.7, smooth=1.0, epsilon=1e-6, focal_alpha=0.25, focal_gamma=2.0):
        super().__init__()
        self.pos_weight = torch.tensor([pos_weight])  # Reduced from 236
        self.dice_weight = dice_weight  # Higher Dice emphasis
        self.smooth = smooth
    ...
    def forward(self, inputs, targets):
        ...
        # Focal Loss instead of simple BCE (better for extreme imbalance)
        ...
        if target_sum < 5:  # Need at least 5 fire pixels for meaningful Dice
            return focal_loss
        
        # IMPROVED Dice loss calculation
        ...
        # Calculate Dice with proper weighting for positive class
        intersection = (inputs_flat * targets_flat).sum()
        union = inputs_flat.sum() + targets_flat.sum()
        dice_score = (2. * intersection + self.smooth) / (union + self.smooth)
        dice_loss = 1 - dice_score
        
        # Combined loss with higher emphasis on Dice
        ...
        if not torch.isfinite(combined_loss):
            ...
            return focal_loss  # Fallback to Focal only
...
class OfficialFireTrainer:
    """Trainer with mixed precision and optimized settings for high-end GPU"""
    ...
    def __init__(self, model, config=None, device=None):
        ...
        self.criterion = ImprovedDiceBCELoss(
            ...
            dice_weight=0.7,  # Higher Dice emphasis
        ...
        # MIXED PRECISION TRAINING - Major speedup for 4070Ti
    ...
    def train_epoch(self, train_loader):
        """Training epoch with mixed precision and batch monitoring"""
        self.model.train()
        epoch_loss = 0.0
        ...
        for batch_idx, (data, target) in enumerate(tqdm(train_loader, desc="Training")):
            ...
            # Gradient clipping for stability (unscaled gradients)
            ...
            # Memory cleanup less frequent due to larger batch size
            if batch_idx % 20 == 0:
    ...
    def validate(self, val_loader):
        """Validation with mixed precision and official AP/AUPRC metrics"""
        self.model.eval()
        val_loss = 0.0
        all_predictions = []
        all_targets = []
